...
def run_tests() -> None:
    ...
    def _number_of_trades() -> None:
        ...
        for name, n_trades in sorted(strategies_n_trades.items(), key=lambda item: item[1], reverse=True):
            print(f'{name}: {n_trades}')
    ...
    # Calculate the total number of trades (across every test condition) each strategy made
    _number_of_trades()
