...
class RandomForestStrategy(Strategy):
    def __init__(self, model_name: str, starting_idx: int = 1,
        ...
        super().__init__(starting_idx, data_format_function, percent_to_risk, 'RandomForest')
        ...
        self.model = RandomForest(model_name)
    ...
    def place_trade(self, curr_idx: int, strategy_data: DataFrame, currency_pair: str, account_balance: float) -> \
        ...
        x = strategy_data.loc[
            strategy_data.index[curr_idx - 1], ['bid_pips_down', 'bid_pips_up', 'ask_pips_down', 'ask_pips_up', 'rsi',
                                                'rsi_sma', 'adx', 'chop', 'vo', 'qqe_up', 'qqe_down', 'qqe_val',
                                                'rsi_up', 'adx_large', 'chop_small', 'vo_positive', 'squeeze_on',
                                                'macd', 'macdsignal']]
